...
CallbackReturn HQPController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/)
{
    ...
    if (!use_estimator) {
        ...
        rclcpp::QoS out_qos(QUEUE_SIZE);
        out_qos.reliability(rclcpp::ReliabilityPolicy::BestEffort);
        estimated_pose_subscription_ = get_node()->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
            "/pronto_estimator/pose", out_qos,
            [this](const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) -> void
            {
                q_.head(7) << msg-> pose.pose.position.x, msg-> pose.pose.position.y, msg-> pose.pose.position.z, //msg->position.x, msg->position.y, msg->position.z,
                              msg-> pose.pose.orientation.x, msg-> pose.pose.orientation.y, msg-> pose.pose.orientation.z, msg-> pose.pose.orientation.w; //msg->orientation.x, msg->orientation.y, msg->orientation.z, msg->orientation.w;
            }
        );

        estimated_twist_subscription_ = get_node()->create_subscription<geometry_msgs::msg::TwistWithCovarianceStamped>(
            "/pronto_estimator/twist", out_qos,
            [this](const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr msg) -> void
            {
                v_.head(6) << msg-> twist.twist.linear.x, msg-> twist.twist.linear.y, msg-> twist.twist.linear.z,
